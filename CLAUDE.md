...
- [src/aafunc.ts](src/aafunc.ts) — defines the catalog of processing functions available in the query editor UI (mirrored in `testdata/functions/defs.json`; see Processing functions below).
...
- **Query modes**: backend-executed vs. frontend-executed queries are two parallel implementations of the same semantics (target parsing, operators, alias, functions, extrapolation). When changing query behavior, check whether the equivalent Go (`pkg/archiverappliance`, `pkg/functions`) and TypeScript (`src/query.ts`, `src/dataProcessor.ts`, `src/aafunc.ts`) code needs matching updates.
  - Known divergence: when the samples of a waveform differ in length (its NORD changes between samples), the index and timeseries array formats lay every sample out to the longest one, with nulls where a sample has no element, in the frontend (`src/responseParse.ts`), but cut every sample to the shortest one in the backend (`pkg/models/arrays.go`). The backend was left as it is because nullable fields made building frames slower for the common case of samples of equal length. Only these two formats differ, and only when lengths differ.
...
- **Processing functions**: applied server-side (`pkg/functions`) or client-side (`dataProcessor.ts`), configured through the function-chain UI in `src/components/Function*.tsx`. Function definitions/param schemas live in `src/aafunc.ts` (frontend) and must match the dispatch table in `pkg/functions/functions_handler.go` (backend) by name.
  - The results of both implementations are pinned by the shared vectors in [testdata/functions/](testdata/functions/), run by [pkg/functions/vectors_test.go](pkg/functions/vectors_test.go) (`mage atest TestFunctionVector`) and [src/specs/vectors.test.ts](src/specs/vectors.test.ts) (`yarn jest src/specs/vectors.test.ts`). Where the two disagree, the backend is the reference, since alerting only runs there.
  - To change what a function returns, change the vector first and make both implementations pass it; write per-implementation tests only for what a vector cannot express (nil samples, which only the backend produces, and returned errors).
  - `defs.json` is checked against the `src/aafunc.ts` catalog, and every non-Options entry in it against the backend dispatch, so adding a function means adding it to all three.
